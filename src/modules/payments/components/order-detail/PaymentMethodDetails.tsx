 ...
 */
export const PaymentMethodDetails = ({ sourceOfFunds }: PaymentMethodDetailsProps) => {
    if (!sourceOfFunds) return null;

    // Determine payment type and render appropriate component
    if (isValuPayment(sourceOfFunds)) {
        return <ValuPaymentDetails sourceOfFunds={sourceOfFunds} />;
    }

    if (isCashPayment(sourceOfFunds)) {
        return <CashPaymentDetails sourceOfFunds={sourceOfFunds} />;
    }

    if (isWalletPayment(sourceOfFunds)) {
        return <WalletPaymentDetails sourceOfFunds={sourceOfFunds} />;
    }

    if (isCardPayment(sourceOfFunds)) {
        return <CardPaymentDetails sourceOfFunds={sourceOfFunds} />;
    }
